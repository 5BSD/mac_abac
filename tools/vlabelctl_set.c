 ...
 * All rights reserved.
 *
 * vlabelctl - Set command handlers
 *
...
{
	fprintf(stderr,
	    "usage: vlabelctl set <command> [arguments]\n"
	    "\n"
	    "Commands:\n"
	    ...
	    "\n"
	    "Examples:\n"
	    "  vlabelctl set disable 1\n"
	    "  vlabelctl set enable 1-10\n"
	    "  vlabelctl set swap 0 1000\n"
	    "  vlabelctl set list 0-100\n"
	);
	exit(EX_USAGE);
}
...
cmd_set(int argc, char *argv[])
{
	...
	if (strcmp(argv[0], "enable") == 0) {
		...
		if (vlabel_syscall(VLABEL_SYS_SET_ENABLE, &range) < 0)
		...
		if (vlabel_syscall(VLABEL_SYS_SET_DISABLE, &range) < 0)
		...
		if (vlabel_syscall(VLABEL_SYS_SET_SWAP, sets) < 0)
		...
		if (vlabel_syscall(VLABEL_SYS_SET_MOVE, sets) < 0)
		...
		if (vlabel_syscall(VLABEL_SYS_SET_CLEAR, &set) < 0)
			err(EX_OSERR, "SET_CLEAR");

		printf("cleared set %u\n", set);

	} else if (strcmp(argv[0], "list") == 0) {
		struct vlabel_set_list_arg list_arg;
		...
		while (start <= end) {
			...
			if (vlabel_syscall(VLABEL_SYS_SET_LIST, &list_arg) < 0)
