...
# - security.mac.abac.enabled
# - security.mac.abac.mode
# - security.mac.abac.default_policy
# - security.mac.abac.extattr_name
#
...
ORIG_ENABLED=$(sysctl -n security.mac.abac.enabled)
ORIG_MODE=$(sysctl -n security.mac.abac.mode)
ORIG_DEFAULT=$(sysctl -n security.mac.abac.default_policy)
ORIG_EXTATTR=$(sysctl -n security.mac.abac.extattr_name)

cleanup() {
	rm -f "$TEST_FILE" 2>/dev/null || true
	sysctl security.mac.abac.enabled=$ORIG_ENABLED >/dev/null 2>&1 || true
	sysctl security.mac.abac.mode=$ORIG_MODE >/dev/null 2>&1 || true
	sysctl security.mac.abac.default_policy=$ORIG_DEFAULT >/dev/null 2>&1 || true
	sysctl security.mac.abac.extattr_name="$ORIG_EXTATTR" >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
# ===========================================
info "=== security.mac.abac.enabled ==="
...
if sysctl security.mac.abac.enabled=0 >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.enabled)
	if [ "$VAL" = "0" ]; then
		pass "set enabled=0"
	else
...
if sysctl security.mac.abac.enabled=1 >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.enabled)
	if [ "$VAL" = "1" ]; then
		pass "set enabled=1"
	else
...
# ===========================================
info ""
info "=== security.mac.abac.mode ==="

run_test
info "Test: Read mode"
if sysctl -n security.mac.abac.mode >/dev/null 2>&1; then
	pass "read mode"
else
	fail "read mode"
fi
...
if sysctl security.mac.abac.mode=0 >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.mode)
	if [ "$VAL" = "0" ]; then
		pass "set mode=0"
	else
...
if sysctl security.mac.abac.mode=1 >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.mode)
	if [ "$VAL" = "1" ]; then
		pass "set mode=1"
	else
...
sysctl security.mac.abac.mode=1 >/dev/null 2>&1
SYSCTL_VAL=$(sysctl -n security.mac.abac.mode)
...
# ===========================================
info ""
info "=== security.mac.abac.default_policy ==="
...
if sysctl security.mac.abac.default_policy=0 >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.default_policy)
	if [ "$VAL" = "0" ]; then
...
# ===========================================
info ""
info "=== security.mac.abac.extattr_name ==="

run_test
info "Test: Read extattr_name"
EXTATTR=$(sysctl -n security.mac.abac.extattr_name 2>/dev/null)
if [ -n "$EXTATTR" ]; then
...
if sysctl security.mac.abac.extattr_name="test_label" >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.extattr_name)
...
if sysctl security.mac.abac.extattr_name="mac_abac" >/dev/null 2>&1; then
	VAL=$(sysctl -n security.mac.abac.extattr_name)
...
sysctl security.mac.abac.extattr_name="custom_attr" >/dev/null 2>&1
# Create test file
echo "test" > "$TEST_FILE"
...
# Restore
sysctl security.mac.abac.extattr_name="mac_abac" >/dev/null 2>&1
...
if sysctl -n security.mac.abac.checks >/dev/null 2>&1; then
	pass "read checks"
else
	fail "read checks"
fi
...
if sysctl -n security.mac.abac.allowed >/dev/null 2>&1; then
	pass "read allowed"
else
	fail "read allowed"
fi
...
if sysctl -n security.mac.abac.denied >/dev/null 2>&1; then
	pass "read denied"
else
	fail "read denied"
fi
...
if sysctl -n security.mac.abac.rule_count >/dev/null 2>&1; then
...
if sysctl -n security.mac.abac.labels_read >/dev/null 2>&1; then
...
if sysctl -n security.mac.abac.labels_default >/dev/null 2>&1; then
...
if sysctl -n security.mac.abac.labels_allocated >/dev/null 2>&1; then
...
if sysctl -n security.mac.abac.labels_freed >/dev/null 2>&1; then
...
if sysctl -n security.mac.abac.parse_errors >/dev/null 2>&1; then
...
if sysctl security.mac.abac.checks=0 2>/dev/null; then
...
run_test
info "Test: sysctl security.mac.abac shows all tunables"
OUTPUT=$(sysctl security.mac.abac 2>&1)
MISSING=""
