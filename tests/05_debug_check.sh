#!/bin/sh
#
# Test: ABAC debug check (ABAC_OP_DEBUG)
#
# Tests that abac_proc_check_debug() properly evaluates rules for ptrace/debug
# operations between processes with different labels.
#
# Rule syntax: mac_abac_ctl rule add "action operation subject -> object"
...
#
# Examples:
#   mac_abac_ctl rule add "allow debug type=admin -> *"
#   mac_abac_ctl rule add "deny debug * -> type=protected"
#   mac_abac_ctl rule add "allow signal type=supervisor -> type=worker"
#
# Prerequisites:
# - Must be run as root
...
if [ ! -x "$MAC_ABAC_CTL" ]; then
    fail "mac_abac_ctl not found or not executable: $MAC_ABAC_CTL"
fi
...
echo "============================================"
echo "ABAC Debug Check Test (ABAC_OP_DEBUG)"
echo "============================================"
echo ""

# ---------------------------------------------------------------------------
# ACTION: add_debug_rule
# Verify mac_abac_ctl recognizes 'debug' as a valid operation
# Rule: "allow debug * -> *" (allow any process to debug any process)
# ---------------------------------------------------------------------------
info "ACTION: add_debug_rule"
RULE="allow debug * -> *"
info "  Rule: $RULE"
if ! $MAC_ABAC_CTL rule add "$RULE"; then
    fail "Could not add rule: $RULE"
fi
pass "debug operation recognized by mac_abac_ctl"
$MAC_ABAC_CTL rule clear 2>/dev/null || true

# ---------------------------------------------------------------------------
# ACTION: add_signal_rule
# Verify mac_abac_ctl recognizes 'signal' as a valid operation
...
# Verify mac_abac_ctl recognizes 'sched' as a valid operation
...
RULE_OUTPUT=$($MAC_ABAC_CTL rule list)
...
RULE_COUNT=$($MAC_ABAC_CTL rule list | wc -l)
...
$MAC_ABAC_CTL rule add "allow signal type=supervisor -> *"
$MAC_ABAC_CTL rule add "allow sched type=scheduler -> *"

RULES_OUTPUT=$($MAC_ABAC_CTL rule list)
info "  Rules:"
...
# Verify ABAC_OP_ALL bitmask includes debug/signal/sched
# Rule: "allow all * -> *"
# ---------------------------------------------------------------------------
...
echo ""
echo "Example rules:"
echo "  mac_abac_ctl rule add \"deny debug * -> type=protected\""
echo "  mac_abac_ctl rule add \"allow debug type=admin -> *\""
echo "  mac_abac_ctl rule add \"deny debug * -> * ctx:sandboxed=true\""
echo "  mac_abac_ctl rule add \"allow signal type=supervisor -> type=worker\""
echo ""
